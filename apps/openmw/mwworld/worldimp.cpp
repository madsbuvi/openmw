...
namespace MWWorld
{
        ...
        SceneUtil::WorkQueue* workQueue, SceneUtil::UnrefQueue& unrefQueue, std::unique_ptr<MWRender::Camera> camera)
    {
        ...
        //## VR_PATCH BEGIN
        if (!camera)
            camera = std::make_unique<MWRender::Camera>(viewer->getCamera());
...
//## VR_PATCH BEGIN
    float World::getActivationDistancePlusTelekinesis()
    {
        ...
        const float telekinesisMagnitude = player.getClass()
                                               ...
                                               .getMagnitude();
        return getMaxActivationDistance() + feetToGameUnits(telekinesisMagnitude);
    }
//## VR_PATCH END
...
//## VR_PATCH BEGIN
    ...
    {
        auto* anim = dynamic_cast<MWVR::VRAnimation*>(mRendering->getAnimation(MWMechanics::getPlayer()));
        if (anim)
            anim->enablePointers(left, right);
    }
