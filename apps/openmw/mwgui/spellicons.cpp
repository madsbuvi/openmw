...
//## VR_PATCH BEGIN
        std::vector<MyGUI::ImageBox*> vrImages;
//## VR_PATCH END
...
//## VR_PATCH BEGIN
        if (VR::getVR())
        {
            // Effect box grows to the right; reverse positions so existing icons stay put.
            int reverseOffset = horizontalOffset;
            for (auto* image : vrImages)
            {
                reverseOffset -= size;
                image->setPosition(reverseOffset, verticalOffset);
            }
        }
//## VR_PATCH END
